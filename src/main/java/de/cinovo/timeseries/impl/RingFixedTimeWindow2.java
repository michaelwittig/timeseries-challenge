...
/**
 * Optimization: Expand strategy
 * 
 * @author mwittig
 * 
 */
public final class RingFixedTimeWindow2 implements IFixedTimeWindow {
	 ...
	 */
	public RingFixedTimeWindow2(final long windowLength, final int maxSize, final ExpandStrategy expandStrategy) {
	...
	public static final class Ring implements ITimeSeries {
		...
		private double sumquadrat = 0.0d;
		...
		public void add(long time, float value) {
			...
			if (((nextHead < this.maxSize) && (nextHead != this.ringTail)) || (this.ringHead == -1)) {
				...
				if (this.expandStrategy == ExpandStrategy.override) {
					...
					final int newMaxSize = this.maxSize * 2;
					final long[] newTimes = new long[newMaxSize];
					final float[] newValues = new float[newMaxSize];
					System.arraycopy(this.times, 0, newTimes, 0, this.times.length);
					System.arraycopy(this.values, 0, newValues, 0, this.values.length);
					this.maxSize = newMaxSize;
					this.times = newTimes;
					this.values = newValues;
					this.add(time, value); // readd simple implementation :)
				} else {
			...
			if ((this.cachedMaximum == null) || ((this.cachedMaximum != Ring.CLEARED) && FloatHelper.greaterThan(value, this.cachedMaximum.value(), AFixedTimeWindowTest.PRECISION))) {
				this.cachedMaximum = new TimeSeriesPair(time, value); // we have a new maximum
			}
			if ((this.cachedMinimum == null) || ((this.cachedMinimum != Ring.CLEARED) && FloatHelper.lessThan(value, this.cachedMinimum.value(), AFixedTimeWindowTest.PRECISION))) {
		...
		private void refreshCacheVarDev() {
			if (this.size > 0) {
				...
				this.cachedVariance = (float) (this.sumquadrat / this.size) - (avg * avg);
