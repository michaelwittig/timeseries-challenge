...
public final class DeltaFixedTimeWindow3 implements IFixedTimeWindow {
	...
	private static final class Wrapper implements ITimeSeries {
		...
		public void add(final TimeSeriesPair pair) {
			...
			if ((this.cachedMaximum == null) || ((this.cachedMaximum != this.CLEARED) && FloatHelper.greaterThan(value, this.cachedMaximum.value(), AFixedTimeWindowTest.PRECISION))) {
				this.cachedMaximum = new TimeSeriesPair(time, value); // we have a new maximum
			}
			if ((this.cachedMinimum == null) || ((this.cachedMinimum != this.CLEARED) && FloatHelper.lessThan(value, this.cachedMinimum.value(), AFixedTimeWindowTest.PRECISION))) {
		...
		public void delete(final int deletes_length) {
			for (int i = 0; i < Math.min(this.deletes.length, deletes_length); i++) {
				...
				this.sum -= value;
				if ((this.cachedMaximum != null) && (this.cachedMaximum != this.CLEARED) && FloatHelper.equals(value, this.cachedMaximum.value(), AFixedTimeWindowTest.PRECISION)) {
					this.cachedMaximum = this.CLEARED; // we lost the maximum. we have to check all values to find the new minimum.
				}
				if ((this.cachedMinimum != null) && (this.cachedMinimum != this.CLEARED) && FloatHelper.equals(value, this.cachedMinimum.value(), AFixedTimeWindowTest.PRECISION)) {
